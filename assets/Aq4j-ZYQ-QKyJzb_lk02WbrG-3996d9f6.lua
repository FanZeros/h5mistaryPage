...
---@class FragmentedThumb : Widget
---@field shuffle_ number[]
local FragmentedThumb = Widget:Extend("FragmentedThumb")
...
---@param props table
function FragmentedThumb:Init(props)
    props = props or {}
    props.width = props.width or "100%"
    props.height = props.height or "100%"
    props.position = props.position or "absolute"
    props.top = props.top or 0
    props.left = props.left or 0
    props.right = props.right or 0
    props.bottom = props.bottom or 0
    Widget.Init(self, props)
    if not self.shuffle_ then
        local gridSize = props.gridSize or 3
...
---@param nvg NVGContextWrapper
function FragmentedThumb:Render(nvg)
    local l = self:GetAbsoluteLayout()
    local imgPath = self.props.imagePath or ""
    local gridSize = self.props.gridSize or 3
    local tiles = self.shuffle_ or {}
    if imgPath == "" or l.w <= 0 or l.h <= 0 then
        return
    end
    if fragImgCache_.path ~= imgPath or fragImgCache_.handle < 0 then
        fragImgCache_.handle = nvgCreateImage(nvg, imgPath, 0)
        fragImgCache_.path = imgPath
    end
    local handle = fragImgCache_.handle
    ...
    for slot = 1, gridSize * gridSize do
        ...
        local paint = nvgImagePattern(nvg, ox, oy, cellW * gridSize, cellH * gridSize, 0, handle, 1)
        nvgBeginPath(nvg)
        ...
        nvgBeginPath(nvg)
        nvgRect(nvg, x, y, cellW, cellH)
        nvgStrokeColor(nvg, nvgRGBA(15, 23, 42, 120))
        nvgStrokeWidth(nvg, 1)
        nvgStroke(nvg)
    end
end

local function getJigsawConfig(gameData, itemId)
    if not gameData or not gameData.puzzles or not itemId then
        return nil
    end
    for _, puzzle in pairs(gameData.puzzles) do
        if puzzle.type == "image_grid" and puzzle.linkedAssetId == itemId then
            local gridSize = puzzle.gridSize or 3
            if GameState.GetDifficulty() == "STANDARD" then
                gridSize = gridSize + 1
            end
            local solved = puzzle.rewardFlag and GameState.GetFlag(puzzle.rewardFlag) == true
            return { gridSize = gridSize, solved = solved }
        end
    end
    return nil
end
...
function UI_Game.IsInspecting()
    return inspectingItemId_ ~= nil
end
...
function UI_Game.Create(gameData, gamePhase)
    ...
    -- 对应 H5 z-[90]：队列未播完时全屏拦截点击。
    -- 谜题 / 物品查看 / 广告 / 胜利打开时不盖住，避免吃掉确认按钮。
    local blockAdvanceOverlay = (puzzleWidget ~= nil)
        or (inspectingItemId_ ~= nil)
        or (Hint.IsAdPromptOpen and Hint.IsAdPromptOpen())
        or (gamePhase == "victory")
    if UI_Message.IsPlaying() and not blockAdvanceOverlay then
        children[#children + 1] = UI.Panel {
            id = "msgAdvanceOverlay",
            position = "absolute",
            top = 0, left = 0, right = 0, bottom = 0,
...
function CreateBottomPanel(gameData, scene)
    ...
    for _, entry in ipairs(interactables) do
        ...
        if isRevealed then
            ...
            elseif locked then
                -- 待使用物品：金锁高亮，避免被误认为已完成灰态
                fontColor = { 255, 236, 153, 255 }
                bgColor = { 113, 63, 18, 220 }
                borderColor = { 245, 193, 67, 220 }
            elseif isAudio then
            ...
            elseif isPuzzle then
                ...
                bgColor = { 16, 110, 80, 220 }      -- 提高对比，避免未完成看起来像灰掉
                borderColor = { 52, 211, 153, 180 }
            end
    ...
    if #inventory == 0 then
        ...
        for _, itemId in ipairs(inventory) do
            ...
            if item then
                ...
                -- 缩略图区（PNG 优先，未解拼图显示碎片，无图回退文字）
                local jigsawCfg = getJigsawConfig(gameData, itemId)
                local showFragments = jigsawCfg and not jigsawCfg.solved and imgPath ~= ""
                local thumbChildren = {}
                if showFragments then
                    thumbChildren[#thumbChildren + 1] = FragmentedThumb {
                        imagePath = imgPath,
                        gridSize = jigsawCfg.gridSize,
                    }
                elseif imgPath == "" then
                ...
                itemWidgets[#itemWidgets + 1] = UI.Panel {
                    ...
                    backgroundImage = (imgPath ~= "" and not showFragments) and imgPath or nil,
                    ...
                    children = (function()
                        ---@type table[]
                        local kids = {}
                        for i = 1, #thumbChildren do
                            kids[#kids + 1] = thumbChildren[i]
                        end
                        kids[#kids + 1] = UI.Panel {
                            position = "absolute",
...
function CreateItemInspectModal(item)
    ...
    local jigsawCfg = getJigsawConfig(gameData, item.id)
    local showFragments = jigsawCfg and not jigsawCfg.solved and imgPath ~= ""
...
function CreateVictoryOverlay(gameData)
    ...
    local kids = {
        ...
        UI.Label {
            text = meta.endingMessage or "恭喜通关！",
            fontSize = 14,
            fontColor = { 203, 213, 225, 255 },
            textAlign = "center",
            width = "100%",
    ...
    if not Storage.HasRated() then
        kids[#kids + 1] = UI.Panel {
            width = "100%",
            padding = 12,
            gap = 8,
            backgroundColor = { 69, 39, 16, 220 },
            borderRadius = 12,
            borderWidth = 1,
            borderColor = { 217, 119, 6, 160 },
            alignItems = "center",
            children = {
                UI.Label {
                    text = "游戏如何？",
                    fontSize = 15,
                    fontColor = { 253, 230, 138, 255 },
                    textAlign = "center",
                },
                UI.Label {
                    text = "随机解锁 3 个故事。前往评价可支持我们。",
                    fontSize = 12,
                    fontColor = { 252, 211, 77, 230 },
                    textAlign = "center",
                    width = "100%",
                    whiteSpace = "normal",
                    wordBreak = "break-word",
                ...
                UI.Button {
                    ...
                    width = "100%",
                    onClick = function(self)
                        if Storage.HasRated() then
                            return
                        end
                        Storage.MarkRated()
                        local unlocked = Storage.RewardUnlockGames(GetGameList and GetGameList() or {})
                        if #unlocked > 0 then
                            local names = {}
                            for i, g in ipairs(unlocked) do
                                names[i] = g.name or g.id
                            end
                            UI_Message.QueueMessages({ "已解锁：" .. table.concat(names, "、") })
                        else
                            UI_Message.QueueMessages({ "感谢评价！" })
                        end
