...
-- ============================================================================
-- 评价奖励（对应 H5 hasRated / rewardUnlockGames）
-- ============================================================================

function Storage.HasRated()
    return data_.preferences.hasRated == true
end

function Storage.MarkRated()
    data_.preferences.hasRated = true
    Storage.Save()
end

--- 从尚未解锁的付费故事中随机解锁最多 3 个
---@param allGames table[]
---@return table[] 被解锁的游戏条目
function Storage.RewardUnlockGames(allGames)
    local locked = {}
    for _, game in ipairs(allGames or {}) do
        if not game.free and not Storage.IsGameUnlocked(game.id, false) then
            locked[#locked + 1] = game
        end
    end
    for i = #locked, 2, -1 do
    ...
    local count = math.min(3, #locked)
    local unlocked = {}
    for i = 1, count do
        Storage.UnlockGame(locked[i].id)
        unlocked[#unlocked + 1] = locked[i]
    end
    return unlocked
end
